...
class MagicTree {
  ...
  /**
   * Prints a single Magic Tree
   *
   * @param int $tree The tree index
   * @param int $age The tree age
   * @return void
   */
  function printTree(int $tree, int $age): void {
    ...
    for ($i=0; $i<$buds; $i++) {
      if ($i === 0) {
        // Top row with alternating Y markers
        echo str_repeat(' Y ', (int) ($buds / 2)) . "\n";
      } elseif ($i === $buds - 1) {
        // Last row with center | symbol
        echo str_repeat("-", $buds - 1) . '|' . str_repeat("-", $buds - 1) . "\n";
      } else {
        // Middle rows with arcs and spaces dynamically
        $outerSpaces = str_repeat(' ', $i);
        $innerSpaces = str_repeat(' ', $buds - $i - 1);
        echo $outerSpaces . "\\" . $innerSpaces . "/" . $outerSpaces . "\n";
      }
    }
  }
}
